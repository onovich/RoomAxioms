import { describe, expect, it } from 'vitest'
import type { RuleDefinition } from '@room-axioms/domain'

import { comparatorText, ruleChip, rulePlainText, ruleSemantics } from './scopeText'

describe('rule text helpers', () => {
  it('renders compact global count chips with anomaly wording', () => {
    expect(ruleChip(globalRule())).toBe('有 2 处异常区域')
    expect(rulePlainText(globalRule())).toBe('本现场有 2 处异常区域。')
    expect(comparatorText('gte', 1)).toBe('>= 1')
    expect(comparatorText('lte', 0)).toBe('<= 0')
  })

  it('renders one-way local rule semantics', () => {
    const rule = localRule()

    expect(ruleChip(rule)).toBe('周围一圈：有 1 处异常区域')
    expect(rulePlainText(rule)).toBe('镜子的周围一圈：有 1 处异常区域。')
    expect(ruleSemantics(rule)).toBe('镜子的周围一圈：有 1 处异常区域。')
  })

  it('renders zero-count local rules as natural exclusion sentences', () => {
    const rule: RuleDefinition = {
      id: 'R3',
      type: 'forEachCount',
      subject: 'bottle',
      scope: { kind: 'orthogonal' },
      target: 'guest',
      count: { op: 'eq', value: 0 },
      presentation: { title: 'Bottle' },
    }

    expect(ruleChip(rule)).toBe('上下左右邻格：没有异常区域')
    expect(rulePlainText(rule)).toBe('异常区域不在酒瓶的上下左右邻格。')
  })

  it('renders region count rules with the player-facing title', () => {
    const rule: RuleDefinition = {
      id: 'ZR1',
      type: 'regionCount',
      regionId: 'north-wing',
      target: 'guest',
      count: { op: 'eq', value: 1 },
      presentation: { title: '北侧登记区' },
    }

    expect(ruleChip(rule)).toBe('北侧登记区：有 1 处异常区域')
    expect(rulePlainText(rule)).toBe('北侧登记区区域：有 1 处异常区域。')
  })

  it('renders line count rules with row labels', () => {
    const rule: RuleDefinition = {
      id: 'LR1',
      type: 'lineCount',
      scope: { kind: 'row', index: 1 },
      target: 'guest',
      count: { op: 'lte', value: 1 },
      presentation: { title: 'Row limit' },
    }

    expect(ruleChip(rule)).toBe('第 2 行：最多有 1 处异常区域')
    expect(rulePlainText(rule)).toBe('第 2 行：最多有 1 处异常区域。')
  })

  it('renders anchor count rules with the player-facing title', () => {
    const rule: RuleDefinition = {
      id: 'AR1',
      type: 'anchorCount',
      anchorId: 'known-bottle',
      scope: { kind: 'orthogonal' },
      target: 'guest',
      count: { op: 'eq', value: 0 },
      presentation: { title: '酒瓶参照物' },
    }

    expect(ruleChip(rule)).toBe('酒瓶参照物的上下左右邻格：没有异常区域')
    expect(rulePlainText(rule)).toBe('酒瓶参照物的上下左右邻格：没有异常区域。')
  })

  it('sanitizes reviewed presentation flavor for shipped rule text', () => {
    const rule: RuleDefinition = {
      id: 'ZR2',
      type: 'regionCount',
      regionId: 'north-wing',
      target: 'guest',
      count: { op: 'eq', value: 0 },
      presentation: {
        title: '北侧登记区',
        flavor: '北侧登记区里没有未登记访客。',
      },
    }

    expect(rulePlainText(rule)).toBe('北侧登记区里没有异常区域。')
    expect(ruleSemantics(rule)).toBe('北侧登记区里没有异常区域。')
  })

  it('renders record-set rules without internal assignment details', () => {
    const rule: RuleDefinition = {
      id: 'CR1',
      type: 'recordSet',
      recordIds: ['card-a', 'card-b'],
      falseRecords: { op: 'eq', value: 1 },
      presentation: { title: 'Polluted records' },
    }

    expect(ruleChip(rule)).toBe('污染记录：恰好 1 张记录可能为假')
    expect(rulePlainText(rule)).toBe('污染记录：恰好 1 张记录可能为假。')
  })
})

function globalRule(): RuleDefinition {
  return {
    id: 'R1',
    type: 'globalCount',
    target: 'guest',
    count: { op: 'eq', value: 2 },
    presentation: { title: 'Guests' },
  }
}

function localRule(): RuleDefinition {
  return {
    id: 'R2',
    type: 'forEachCount',
    subject: 'mirror',
    scope: { kind: 'adjacent' },
    target: 'guest',
    count: { op: 'eq', value: 1 },
    presentation: { title: 'Mirror' },
  }
}
