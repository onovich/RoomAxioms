...
当前 Figma 原型是 1920x1080 宽屏布局；正式实现要支持桌面优先，同时保留移动端可用退化策略。移动端可以继续使用当前移动 tab 方案或压缩布局，但不能让核心操作不可达。
...
- Round 3：建立正式玩家页的新 shell：顶栏、左规则、中地图、右记录、底部 VN overlay 的稳定区域。
...
- Round 11：响应式与文本适配：1366x768、1920x1080、390x844。
...
- 正式玩家路线默认进入 Figma 主稿方向的《未登记现场》UI，而不是旧三栏卡片风格。
