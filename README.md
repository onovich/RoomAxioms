...
- Guide: none
- Active budget: none
- Current check status: Phase 37 accepted after final workbench information-architecture repair; arbitrary object rule semantics remain a deliberate next-phase migration.
...
- Latest phase result: Phase 37 repaired the workbench after hands-on feedback: blue low-contrast workbench text was replaced with high-contrast local tokens, diagnostics run through a cancellable worker path, map actions are placed in the Library/Board headers, case title and difficulty live in the top title area, the old metadata/debug panels are removed, cell objects use a dropdown and manager, the Figure-3-style rule-card list is the single normal rule list, rule create/edit uses a dialog, scope preview is wired where materializable, and low-level diagnostics are hidden or renamed in plain language.
